...
async function openBenchmarkClass(driver) {
	console.log("🎯 Measuring class opening for 'Benchmark Test 1 Do not touch'...");

	// --- LOCATE "Benchmark Test 1 Do not touch" CLASS (same as openClass.js) ---
	...
	// --- START TIMER + CLICK CLASS (same as openClass.js) ---
	...
	// --- WAIT FOR CLASS TO OPEN WITH LEARNER PERFORMANCE DATA (same as openClass.js) ---
...
export async function compareOpenClass(driver) {
	console.log("🔬 Open Class Cache Comparison - Cold vs Warm using dashboard navigation");
	...
	console.log("🔄 Navigating back to dashboard for warm test...");

	try {
		// Try to navigate back to educator dashboard/classes list
		await driver.get(buildEducatorUrl());
		await new Promise(r => setTimeout(r, 2000));

		// Wait for dashboard to load
		await driver.wait(
			until.elementLocated(By.xpath("//*[contains(text(),'Classes') or contains(text(),'Dashboard') or contains(text(),'Educator')]")),
			15000
		);
		console.log("✅ Back to educator dashboard for warm test");
	} catch (e) {
		console.log("⚠️ Navigation to dashboard failed, trying alternative...");
		// Try alternative navigation - go directly to classes
		await driver.get(buildEducatorUrl() + "#home&t=classes");
		await new Promise(r => setTimeout(r, 2000));
	}

	// Small pause to let dashboard settle
	await new Promise(r => setTimeout(r, 2000));
